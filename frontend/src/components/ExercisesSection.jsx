...
const ExercisesSection = () => {
  ...
  const handlePrev = () => {
    setSlideIndex((prev) =>
      prev === 0 ? Math.max(exercises.length - ITEMS_PER_PAGE, 0) : prev - 1,
    );
  };

  const handleNext = () => {
    setSlideIndex((prev) =>
      prev >= exercises.length - ITEMS_PER_PAGE ? 0 : prev + 1,
    );
  };
  ...
  return (
    <section className="py-24 bg-white text-center">
      <div className="container mx-auto px-6 max-w-[1100px]">
        <h4 className="font-teko text-xfitorange font-bold tracking-[0.3em] text-lg md:text-2xl mb-2 uppercase">
          DETTE TILBYDER VI
        </h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {visibleExercises.map((exercise) => (
            <div
              key={exercise._id}
              className="bg-white border border-gray-100 rounded-[20px] px-8 py-12 shadow-[0_4px_30px_rgba(0,0,0,0.06)] flex flex-col items-center text-center"
            >
              <div className="w-28 h-28 mb-8 flex items-center justify-center">
                <img
                  src={resolveApiAssetUrl(exercise.image)}
                  alt={exercise.title}
              ...
              <h3 className="font-teko text-[28px] font-bold text-xfitgray uppercase mb-4 leading-none">
                {exercise.title}
              </h3>
